...
namespace GitNinja.Gallery.Controllers
{
  public class GitController : Controller
  {
    ...
    {
      if (!App_Start.GitNinja.IsValidRepository(dojo, repo)) return HttpNotFound();
      return new GitStreamResult("{0} --stateless-rpc .", "upload-pack", App_Start.GitNinja.GetRepositoryPath(dojo, repo));
    }
    ...
    {
      if (!App_Start.GitNinja.IsValidRepository(dojo, repo)) return HttpNotFound();
      return new GitStreamResult("{0} --stateless-rpc .", "receive-pack", App_Start.GitNinja.GetRepositoryPath(dojo, repo));
    }

    public ActionResult GetInfoRefs(string dojo, string repo, string service)
    {
      if (!App_Start.GitNinja.IsValidRepository(dojo, repo)) return HttpNotFound();
      return new GitCommandResult("{0} --stateless-rpc --advertise-refs .", GetService(service), App_Start.GitNinja.GetRepositoryPath(dojo, repo));
    }
